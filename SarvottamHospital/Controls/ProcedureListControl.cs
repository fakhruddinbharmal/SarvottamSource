...
{
    public partial class ProcedureListControl : ObjectListControl
    {
        ...
        private Procedure GetSelectedEntityObject()
        {
            return GetSelectedEntity(this.dgvData) as Procedure;
        }

        private void LoadListData(Procedure selected)
        {
            int count = 0;
            this.LoadEntityList<Procedure>(this.dgvData, this.clmName.Index, new Procedures(tstSearch.Text), selected,
                delegate(DataGridViewRow row, Procedure obj)
                {
                    count++;
